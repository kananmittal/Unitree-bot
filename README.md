...
1. **TsPCA (Two-Stream Pooling Channel Attention)**
   ...
   - **Process**:
     ...
     - Each stream: `(B, C, T)` → `(B, C)` (pooling) → MLP → `(B, C)` (attention weights)
     - MLP structure: `Linear(C, C//8) → ReLU → Linear(C//8, C) → Sigmoid`
     - Final weights: average of two streams
     - Applies attention: `x * weights.unsqueeze(2)`
   - **Output**: Attention-weighted MFCC features, shape `(B, C, T)`

2. **CNN Stack (Convolutional Neural Network)**
   - **Architecture**: Stack of 1D convolutional blocks
   - **Default**: 2 blocks with 128 channels each
   - **Conv1DBlock**:
...
3. **BiLSTM (Bidirectional Long Short-Term Memory)**
   ...
   - **Process**:
     ...
     - Temporal pooling: Mean over time dimension → `(B, 2H)`
